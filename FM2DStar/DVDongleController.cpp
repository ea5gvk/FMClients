...
CDVDongleController::CDVDongleController(const std::string& device) :
...
void CDVDongleController::encodeIn(const float* audio, unsigned int length)
{
	assert(audio != NULL);
	assert(length == DSTAR_AUDIO_BLOCK_SIZE);
	...
	for (unsigned int i = 0; i < DSTAR_AUDIO_BLOCK_SIZE; i++) {
		short word = short(audio[i] * 32767.0F);
		*q++ = SWAP_BYTES_16(word);
	}
...
bool CDVDongleController::decodeOut(float* audio, unsigned int length)
{
	assert(audio != NULL);
	assert(length == DSTAR_AUDIO_BLOCK_SIZE);
	...
	for (;;) {
		...
		if (type == RESP_AMBE) {
			...
			short* q = (short*)buffer;
			for (unsigned int i = 0U; i < DSTAR_AUDIO_BLOCK_SIZE; i++) {
				short word = SWAP_BYTES_16(*q);
				q++;
				audio[i] = float(word) / 32768.0F;
			}
			audioFound = true;
		}
...
{
	assert(buffer != NULL);
	assert(length >= DVD_AUDIO_LENGTH_BYTES);
	...
	if (len < 0) {
		LogError("Unable to receive the DVD header: len=%d", len);
		return RESP_ERROR;
	}

	switch (c1) {
		case 0xA0U:		// DVD_AMBE_HEADER
			LogMessage("Re-synchronising with the DV-Dongle");
			buffer[0U] = 0x32U;
			buffer[1U] = c1;
...
CDVDongleController::RESP_TYPE CDVDongleController::processResponse(unsigned char* buffer, unsigned int length)
{
	...
	if (::memcmp(buffer, DVD_AUDIO_HEADER, DVD_HEADER_LEN) == 0) {			// Audio data
		...
		if (len != int(DVD_AUDIO_LENGTH_BYTES)) {
			LogError("Unable to receive the DVD audio data, len=%d", len);
			return RESP_ERROR;
		}
		...
		if (len != int(DVD_AMBE_LENGTH_BYTES)) {
			LogError("Unable to receive the DVD AMBE data, len=%d", len);
			return RESP_ERROR;
		}
		...
		if (len != int(DVD_RESP_STOP_LEN - DVD_HEADER_LEN)) {
			LogError("Unable to receive the DVD start/stop response, len=%d", len);
			return RESP_ERROR;
		}
		...
		LogError("Incorrect response to start/stop request: %02X %02X %02X %02X %02X", buffer[0U], buffer[1U], buffer[2U], buffer[3U], buffer[4U]);
		return RESP_ERROR;
		...
		if (len != int(DVD_RESP_NAME_LEN - DVD_HEADER_LEN)) {
			LogError("Unable to receive the DVD name data, len=%d", len);
			return RESP_ERROR;
		}
		...
		LogError("Incorrect response to name request: %02X %02X %02X %02X %02X...", buffer[0U], buffer[1U], buffer[2U], buffer[3U], buffer[4U]);
		return RESP_ERROR;
	} else if (::memcmp(buffer, DVD_RESP_NOP, DVD_HEADER_LEN) == 0) {		// No op
		return RESP_NOP;
	} else {																// Unknown data
		LogError("Unknown DV Dongle header: %02X %02X", buffer[0U], buffer[1U]);
		return RESP_UNKNOWN;
	}
}
