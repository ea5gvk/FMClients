...
void CSlowDataEncoder::setHeaderData(const CHeaderData& header)
{
	...
	m_header.push_back(buffer[3] = header.getRptCall1()[0]);
	m_header.push_back(buffer[4] = header.getRptCall1()[1]);
	cksum.update(buffer, 5U);

	m_header.push_back(SLOW_DATA_TYPE_HEADER | 5U);
	m_header.push_back(buffer[0] = header.getRptCall1()[2]);
	m_header.push_back(buffer[1] = header.getRptCall1()[3]);
	m_header.push_back(buffer[2] = header.getRptCall1()[4]);
	m_header.push_back(buffer[3] = header.getRptCall1()[5]);
	m_header.push_back(buffer[4] = header.getRptCall1()[6]);
	cksum.update(buffer, 5U);

	m_header.push_back(SLOW_DATA_TYPE_HEADER | 5U);
	m_header.push_back(buffer[0] = header.getRptCall1()[7]);
	m_header.push_back(buffer[1] = header.getRptCall2()[0]);
	m_header.push_back(buffer[2] = header.getRptCall2()[1]);
	m_header.push_back(buffer[3] = header.getRptCall2()[2]);
	m_header.push_back(buffer[4] = header.getRptCall2()[3]);
	cksum.update(buffer, 5U);

	m_header.push_back(SLOW_DATA_TYPE_HEADER | 5U);
	m_header.push_back(buffer[0] = header.getRptCall2()[4]);
	m_header.push_back(buffer[1] = header.getRptCall2()[5]);
	m_header.push_back(buffer[2] = header.getRptCall2()[6]);
	m_header.push_back(buffer[3] = header.getRptCall2()[7]);
	m_header.push_back(buffer[4] = header.getYourCall()[0]);
	cksum.update(buffer, 5U);

	m_header.push_back(SLOW_DATA_TYPE_HEADER | 5U);
	m_header.push_back(buffer[0] = header.getYourCall()[1]);
	m_header.push_back(buffer[1] = header.getYourCall()[2]);
	m_header.push_back(buffer[2] = header.getYourCall()[3]);
	m_header.push_back(buffer[3] = header.getYourCall()[4]);
	m_header.push_back(buffer[4] = header.getYourCall()[5]);
	cksum.update(buffer, 5U);

	m_header.push_back(SLOW_DATA_TYPE_HEADER | 5U);
	m_header.push_back(buffer[0] = header.getYourCall()[6]);
	m_header.push_back(buffer[1] = header.getYourCall()[7]);
	m_header.push_back(buffer[2] = header.getMyCall1()[0]);
	m_header.push_back(buffer[3] = header.getMyCall1()[1]);
	m_header.push_back(buffer[4] = header.getMyCall1()[2]);
	cksum.update(buffer, 5U);

	m_header.push_back(SLOW_DATA_TYPE_HEADER | 5U);
	m_header.push_back(buffer[0] = header.getMyCall1()[3]);
	m_header.push_back(buffer[1] = header.getMyCall1()[4]);
	m_header.push_back(buffer[2] = header.getMyCall1()[5]);
	m_header.push_back(buffer[3] = header.getMyCall1()[6]);
	m_header.push_back(buffer[4] = header.getMyCall1()[7]);
	cksum.update(buffer, 5U);

	m_header.push_back(SLOW_DATA_TYPE_HEADER | 5U);
	m_header.push_back(buffer[0] = header.getMyCall2()[0]);
	m_header.push_back(buffer[1] = header.getMyCall2()[1]);
	m_header.push_back(buffer[2] = header.getMyCall2()[2]);
	m_header.push_back(buffer[3] = header.getMyCall2()[3]);
...
void CSlowDataEncoder::setMessageData(const std::string& message)
{
	if (message.empty())
		return;
	...
	text.append("                    ");
	text.resize(SLOW_DATA_TEXT_LENGTH);
