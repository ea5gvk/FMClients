...
void CDVDongleThread::processDecode()
{
	...
	float audioIn[DSTAR_AUDIO_BLOCK_SIZE];
	bool res = m_dongle->decodeOut(audioIn, DSTAR_AUDIO_BLOCK_SIZE);
	if (!res)
		LogError("An error occurred in decodeOut");

	float audioOut[DSTAR_RADIO_BLOCK_SIZE];
...
{
	float audioIn[DSTAR_RADIO_BLOCK_SIZE];
	::memset(audioIn, 0x00, DSTAR_RADIO_BLOCK_SIZE * sizeof(float));

	m_encodeAudio.getData(audioIn, DSTAR_RADIO_BLOCK_SIZE);

	float audioOut[DSTAR_AUDIO_BLOCK_SIZE];
	...
	if (!res)
		LogError("An error occurred in encodeOut");
