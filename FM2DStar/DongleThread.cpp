...
const float UPSAMPLE_AMP   = 6.0F;
const float DOWNSAMPLE_AMP = 0.9F;

CDongleThread::CDongleThread() :
CThread(),
m_encodeCallback(NULL),
m_decodeCallback(NULL),
m_encodeAudio(DSTAR_RADIO_BLOCK_SIZE * 30U, "Audio Encode Buffer"),
m_decodeData(VOICE_FRAME_LENGTH_BYTES * 30U, "Audio Decode Buffer"),
m_bleep(true),
m_killed(false),
...
unsigned int CDongleThread::writeEncode(const float* audio, unsigned int length)
{
	assert(audio != NULL);
...
{
	// float audio[DSTAR_RADIO_BLOCK_SIZE];
	// unsigned int n;
...
void CDongleThread::upSample(const float* audioIn, float* audioOut)
{
	unsigned int out = 0U;
...
void CDongleThread::downSample(const float* audioIn, float* audioOut)
{
	unsigned int in = 0U;
