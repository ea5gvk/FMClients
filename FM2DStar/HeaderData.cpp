...
m_valid(true)
{
	pad(m_myCall1, ' ', LONG_CALLSIGN_LENGTH);
	pad(m_myCall2, ' ', SHORT_CALLSIGN_LENGTH);
	pad(m_yourCall,' ', LONG_CALLSIGN_LENGTH);
	pad(m_rptCall1,' ', LONG_CALLSIGN_LENGTH);
	pad(m_rptCall2,' ', LONG_CALLSIGN_LENGTH);

	m_myCall1 = m_myCall1.substr(0UL, LONG_CALLSIGN_LENGTH);
	m_myCall2 = m_myCall2.substr(0UL, SHORT_CALLSIGN_LENGTH);
	m_yourCall = m_yourCall.substr(0UL, LONG_CALLSIGN_LENGTH);
	m_rptCall1 = m_rptCall1.substr(0UL, LONG_CALLSIGN_LENGTH);
	m_rptCall2 = m_rptCall2.substr(0UL, LONG_CALLSIGN_LENGTH);
}
...
void CHeaderData::setRptCall2(const std::string& callsign)
{
	m_rptCall2 = callsign;
	pad(m_rptCall2, ' ', LONG_CALLSIGN_LENGTH);
	m_rptCall2 = m_rptCall2.substr(LONG_CALLSIGN_LENGTH);
}
